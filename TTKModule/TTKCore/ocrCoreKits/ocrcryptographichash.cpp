...
OCRCryptographicHash::OCRCryptographicHash()
{
...
QString OCRCryptographicHash::decrypt(const QString &data, const QString &key, Priority p)
{
    QString d = data;
    for(int i=0; i<p; ++i)
    {
...
std::string OCRCryptographicHash::xxteaEncrypt(std::string data, std::string key)
{
...
std::string OCRCryptographicHash::xxteaDecrypt(std::string data,  std::string key)
{
...
QString OCRCryptographicHash::xxteaEncrypt(const QString &data, const QString &key)
{
    return xxteaEncrypt(data.toStdString(), key.toStdString()).c_str();
}

QString OCRCryptographicHash::xxteaDecrypt(const QString &data, const QString &key)
{
    return xxteaDecrypt(data.toStdString(), key.toStdString()).c_str();
}

bool OCRCryptographicHash::isBase64(unsigned char c)
{
    return (isalnum(c) || (c == '+') || (c == '/'));
}

std::string OCRCryptographicHash::base64Encode(unsigned char const* bytes_to_encode, unsigned int in_len)
{
    std::string ret;
    int i = 0, j = 0;
...
std::string OCRCryptographicHash::base64Decode(std::string const& encoded_string)
{
...
void OCRCryptographicHash::xxteaUintEncrypt(xxtea_uint *v, xxtea_uint len, xxtea_uint *k)
{
    xxtea_uint n = len - 1;
...
void OCRCryptographicHash::xxteaUintDecrypt(xxtea_uint *v, xxtea_uint len, xxtea_uint *k)
{
    xxtea_uint n = len - 1;
...
unsigned char *OCRCryptographicHash::fixKeyLength(unsigned char *key, xxtea_uint key_len)
{
...
xxtea_uint *OCRCryptographicHash::xxteaToUintArray(unsigned char *data, xxtea_uint len, int include_length, xxtea_uint *ret_len)
{
...
unsigned char *OCRCryptographicHash::xxteaToByteArray(xxtea_uint *data, xxtea_uint len, int include_length, xxtea_uint *ret_len)
{
    xxtea_uint i, n, m;
...
unsigned char *OCRCryptographicHash::doXxteaEncrypt(unsigned char *data, xxtea_uint len, unsigned char *key, xxtea_uint *ret_len)
{
    unsigned char *result;
...
unsigned char *OCRCryptographicHash::doXxteaDecrypt(unsigned char *data, xxtea_uint len, unsigned char *key, xxtea_uint *ret_len)
{
    unsigned char *result;
    xxtea_uint *v, *k, v_len, k_len;
...
unsigned char *OCRCryptographicHash::xxteaEncrypt(unsigned char *data, xxtea_uint data_len, unsigned char *key, xxtea_uint key_len, xxtea_uint *ret_length)
{
    unsigned char *result;
...
unsigned char *OCRCryptographicHash::xxteaDecrypt(unsigned char *data, xxtea_uint data_len, unsigned char *key, xxtea_uint key_len, xxtea_uint *ret_length)
{
    unsigned char *result;
